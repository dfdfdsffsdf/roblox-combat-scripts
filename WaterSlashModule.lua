...
local WaterAnim = ReplicatedStorage:WaitForChild("Animations"):WaitForChild("Styles"):WaitForChild("Breath"):WaitForChild("Water"):WaitForChild("WaterSlash")
local Effect1 = ReplicatedStorage.Effects.Skills.Water:WaitForChild("SlashSlash")
...
local function getWaterStats(char: Model)
	...
	return waterTable and waterTable[weapon] or { Damage = 5, Stun = 0.2 }
end
...
local function ApplyHitbox(char: Model, clone: Instance, stats: { Damage: number, Stun: number }, hitAnim: Animation)
	...
	hitbox.OnHit:Connect(function(hit, targetHumanoid, hitPos)
		...
		StunHandler.Stun(targetHumanoid, stats.Stun, 5, 0)
...
function module.WaterSlash(char)
	...
	lv.VectorVelocity = Vector3.new(0, 0, -56)
	lv.Parent = hrp
